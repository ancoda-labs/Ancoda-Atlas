...
// translated here and no total is recomputed. A panel whose parts do not add
// up to its stated total is still returned; mergeSitrep leaves the reviewed
 ...
 */
export function parseBulletinFigure(raw) {
...
function rowsIn(pane, dict) {
  ...
  while ((match = re.exec(pane)) !== null) {
    const parsed = parseBulletinFigure(match[3]);
    if (!parsed) continue;
...
function totalFor(html, panel) {
  ...
  return match ? parseBulletinFigure(match[1]) : null;
}
