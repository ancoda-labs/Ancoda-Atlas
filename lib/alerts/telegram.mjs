// Telegram Alerter — Multi-tier hazard alerts, semantic dedup, two-way bot commands
// Atlas becomes a conversational disaster-monitoring agent via Telegram
...
// ─── Alert Tiers ────────────────────────────────────────────────────────────
// FLASH:    Immediate action required — life-safety, time-critical (e.g. damaging earthquake)
// PRIORITY: Important hazard cluster — act within hours (e.g. compound flood/landslide risk)
...
const COMMANDS = {
  ...
  '/brief':     'Get a compact text summary of the latest hazard picture',
...
export class TelegramAlerter {
  ...
  _ruleBasedEvaluation(signals, delta) {
    ...
    const seismicSignal = signals.find(s => s.key === 'seismic_event');
    const quakeSignals = signals.filter(s => ['quakes_24h', 'quakes_7d', 'max_magnitude'].includes(s.key));
    const waterSignals = signals.filter(s => ['weather_alerts', 'extreme_alerts', 'max_rain_5d'].includes(s.key));
    const fireSignals = signals.filter(s => ['thermal_total', 'night_fires', 'worst_aqi'].includes(s.key));
    const reliefSignals = signals.filter(s => ['active_disasters', 'impact_reports'].includes(s.key));

    // FLASH: a damaging earthquake outranks everything else in Nepal
    if (seismicSignal) {
      return {
        shouldAlert: true, tier: 'FLASH', confidence: 'HIGH',
        headline: 'Significant Earthquake Detected',
        reason: 'USGS has recorded an M5.5+ event in the Nepal region. Expect aftershocks and possible infrastructure damage.',
        actionable: 'Check the dashboard for epicentre and depth. Verify highway status and district hospital capacity.',
        signals: ['seismic_event'],
        crossCorrelation: 'USGS seismic',
      };
    }

    // FLASH: compound hazard — shaking and saturation on the same slopes
    const criticalQuake = criticals.some(s => quakeSignals.includes(s));
    const criticalWater = criticals.some(s => waterSignals.includes(s));
    if (criticalQuake && criticalWater) {
      return {
        shouldAlert: true, tier: 'FLASH', confidence: 'HIGH',
        headline: 'Compound Hazard — Rain on Shaken Ground',
        reason: 'Seismic activity and extreme rainfall are escalating together. Slopes loosened by shaking fail under saturation, which is what closed the Araniko highway in 2015.',
        actionable: 'Treat hill-district road access as unreliable. Confirm alternate routing before committing convoys.',
        signals: [...quakeSignals, ...waterSignals].map(s => s.label || s.key).slice(0, 5),
        crossCorrelation: 'seismic + hydro-meteorological',
      };
    }

    // FLASH: an extreme-severity weather alert appearing where there was none
    const extremeNew = signals.find(s => s.key === 'extreme_alerts' && s.direction === 'up');
    if (extremeNew && (extremeNew.severity === 'critical' || extremeNew.severity === 'high')) {
      return {
        shouldAlert: true, tier: 'FLASH', confidence: 'HIGH',
        headline: 'Extreme Weather Alert Active',
        reason: `Extreme-severity weather alerts rose ${extremeNew.from} → ${extremeNew.to}. Violent rainfall and hail at this level drive flash flooding and debris flows within hours.`,
        actionable: 'Cross-check DHM bulletins and confirm downstream district warnings have gone out.',
        signals: [extremeNew.label || extremeNew.key],
        crossCorrelation: 'Open-Meteo severe weather',
      };
    }

    // PRIORITY: ≥2 high/critical hazard signals escalating together
    ...
    if (escalatedHighs.length >= 2) {
      return {
        ...
        headline: `${escalatedHighs.length} Escalating Hazard Signals`,
        reason: `Multiple hazard indicators escalating simultaneously: ${escalatedHighs.map(s => s.label || s.key).slice(0, 3).join(', ')}.`,
        actionable: 'Monitor for continuation. Check whether the trend persists in the next sweep.',
        signals: escalatedHighs.map(s => s.label || s.key).slice(0, 5),
        crossCorrelation: 'multi-hazard',
      };
    }

    // PRIORITY: a new declared response operation
    const newDisaster = reliefSignals.find(s => s.direction === 'up');
    if (newDisaster) {
      return {
        shouldAlert: true, tier: 'PRIORITY', confidence: 'MEDIUM',
        headline: 'New Declared Disaster for Nepal',
        reason: `ReliefWeb active disasters moved ${newDisaster.from} → ${newDisaster.to}. A formal response operation has been opened.`,
        actionable: 'Route district requests through the existing cluster coordination rather than opening a parallel operation.',
        signals: [newDisaster.label || newDisaster.key],
        crossCorrelation: 'UN OCHA ReliefWeb',
      };
    }

    // PRIORITY: fire and smoke moving together
    const escalatingFire = fireSignals.filter(s => s.direction === 'up');
    if (escalatingFire.length >= 2) {
      return {
        shouldAlert: true, tier: 'PRIORITY', confidence: 'MEDIUM',
        headline: 'Fire Activity and Smoke Rising Together',
        reason: `${escalatingFire.map(s => s.label || s.key).join(', ')} all escalating. Detections plus degrading air quality means smoke, not just burn scars.`,
        actionable: 'Expect valley AQI to worsen and hill-airstrip visibility to drop. Check school and flight advisories.',
        signals: escalatingFire.map(s => s.label || s.key).slice(0, 4),
    ...
    if (criticals.length >= 1 || highs.length >= 3) {
      ...
      return {
        ...
        headline: topSignal.label || topSignal.reason || 'Hazard Change Detected',
  ...
  _buildEvaluationPrompt() {
    return `You are Atlas, an emergency alert evaluator for a Nepal natural-disaster monitoring system. You analyze signal deltas from a 5-source hazard sweep (USGS seismic, Open-Meteo weather, NASA FIRMS fire, Open-Meteo air quality, ReliefWeb) and decide whether the user needs a Telegram alert.
...
DO NOT ALERT ON:
- Normal seasonal variation (fire detections in March-May, heavy rain in June-September)
- Continuation of existing trends already flagged in prior sweeps
- Cumulative drift (multiple small moves in the same direction over several sweeps)
- Background seismicity below M4 with no shallow cluster

FLASH (wake the user):
- Any earthquake at M5.5 or above in the Nepal region
- A shallow M5+ within 50km of a major population centre
- Flood or landslide alerts at extreme severity across multiple districts
- A glacial lake outburst flood, or a dam or embankment breach
- Compound hazard: extreme rainfall on slopes with recent seismic activity

PRIORITY (worth reading soon):
- A seismic sequence: 5+ events in 24h with shallow M4.5+ ruptures
- Cumulative rainfall above 200mm over five days at any monitored station
- AQI above 200 in any monitored city
- Fire detections and air quality degrading together — smoke, not just burn scars
- A newly declared disaster response operation for Nepal on ReliefWeb
- Extreme heat in the Terai above 42°C, or a cold wave with reported exposure deaths

ROUTINE (log it, do not push):
- Single-hazard moves at high severity without corroboration
- Source degradation that reduces coverage without a hazard signal behind it
...
Respond with ONLY valid JSON:
{
  ...
  "reason": "2-3 sentences on why this matters for Nepal",
  "actionable": "What the user should do or check",
  "signals": ["signal1", "signal2"],
  "crossCorrelation": "Which hazard layers are confirming each other (e.g. 'seismic + rainfall')"
}`;
  }
  ...
  _buildSignalContext(signals, delta) {
    ...
    const seismicSignals = signals.filter(s => ['quakes_24h', 'quakes_7d', 'max_magnitude', 'seismic_event'].includes(s.key));
    const weatherSignals = signals.filter(s => ['weather_alerts', 'extreme_alerts', 'max_rain_5d'].includes(s.key));
    const fireSignals = signals.filter(s => ['thermal_total', 'night_fires', 'worst_aqi'].includes(s.key));
    const reliefSignals = signals.filter(s => ['active_disasters', 'impact_reports', 'hazard_news'].includes(s.key));
    const categorized = new Set([...seismicSignals, ...weatherSignals, ...fireSignals, ...reliefSignals]);
    const otherSignals = signals.filter(s => !categorized.has(s));

    const fmt = s => `  ${s.label || s.key || s.reason}: ${s.from !== undefined ? `${s.from} → ${s.to}` : 'new signal'}${s.pctChange != null ? ` (${s.pctChange > 0 ? '+' : ''}${s.pctChange}%)` : ''}`;

    if (seismicSignals.length > 0) sections.push('🌐 SEISMIC:\n' + seismicSignals.map(fmt).join('\n'));
    if (weatherSignals.length > 0) sections.push('🌧️ RAIN, FLOOD & LANDSLIDE:\n' + weatherSignals.map(fmt).join('\n'));
    if (fireSignals.length > 0) sections.push('🔥 FIRE & AIR QUALITY:\n' + fireSignals.map(fmt).join('\n'));
    if (reliefSignals.length > 0) sections.push('🚨 RESPONSE & REPORTING:\n' + reliefSignals.map(fmt).join('\n'));
    if (otherSignals.length > 0) sections.push('📌 OTHER:\n' + otherSignals.map(fmt).join('\n'));
