...
const SLASH_COMMANDS = [
  ...
  { name: 'brief',     description: 'Compact hazard summary' },
...
export class DiscordAlerter {
      ...
      try {
        ...
        if (response) {
          ...
          if (response.length > 200) {
            await interaction.editReply({ embeds: [this._embed('Atlas', response, 0x00E5FF)] });
          } else {
  ...
  _buildAlertEmbed(evaluation, delta, tier) {
    ...
    if (embed.setFields) {
      ...
      embed.setFooter({ text: `Atlas Intelligence · ${new Date().toISOString().replace('T', ' ').substring(0, 19)} UTC` });
    } else {
      // Raw embed object for webhook fallback
      embed.fields = fields;
      embed.footer = { text: `Atlas Intelligence · ${new Date().toISOString().replace('T', ' ').substring(0, 19)} UTC` };
    }
  ...
  // ─── Actionable Read Alerts ─────────────────────────────────────────────

  /**
   * Post HIGH confidence, short-horizon reads as actionable alerts.
   * @param {Array} ideas — reads from the sweep (LLM or rule-based)
   */
  async sendActionableIdeas(ideas) {
    ...
    for (const idea of fresh) {
      ...
      const fields = [
        { name: 'Subject', value: this._truncate(idea.ticker || '—', EMBED_LIMITS.fieldValue), inline: true },
      ...
      fields.push({ name: '🎯 How to Verify', value: this._truncate(this._followUp(idea), EMBED_LIMITS.fieldValue), inline: false });

      if (embed.setFields) {
        embed.setFields(fields);
        embed.setFooter({ text: `Atlas Actionable Ideas · ${new Date().toISOString().replace('T', ' ').substring(0, 19)} UTC` });
      } else {
        embed.fields = fields;
        embed.footer = { text: `Atlas Actionable Ideas · ${new Date().toISOString().replace('T', ' ').substring(0, 19)} UTC` };
      }
  ...
  /**
   * Suggest how to verify or act on a read. Nepal has no retail prediction
   * market, so this points at the authority that can confirm the signal
   * rather than at a contract to trade.
   */
  _followUp(idea) {
    const ticker = idea.ticker || '';
    const signals = (idea.signals || []).join(' ');
    const haystack = `${ticker} ${signals} ${idea.rationale || idea.text || ''}`;

    // Seismic
    if (/earthquake|seismic|magnitude|aftershock|M\d/i.test(haystack)) {
      return '🏔️ Confirm with the National Seismological Centre and NDRRMA. Check highway status before assuming road access.';
    }
    // Flood / landslide / monsoon
    if (/flood|landslide|monsoon|rainfall|inundation|cloudburst|saturation|GLOF|glacial/i.test(haystack)) {
      return '🌧️ Cross-check DHM rainfall bulletins and NDRRMA district advisories. Terai inundation and hill road closures follow on different timelines.';
    }
    // Fire
    if (/fire|FRP|burning|thermal|wildfire|forest fire/i.test(haystack)) {
      return '🔥 Cross-check with the Department of Forests and district fire reports. Overnight detections mean the fire is running unchecked.';
    }
    // Air quality
    if (/AQI|PM2\.5|air quality|smoke|inversion|haze/i.test(haystack)) {
      return '😷 Cross-check against Department of Environment monitors. Watch for school closure and Tribhuvan visibility advisories.';
    }
    // Heat and cold
    if (/heat|heatwave|cold wave|shitalahar|frost|temperature/i.test(haystack)) {
      return '🌡️ Check district health-post load. Heat and cold casualties concentrate among outdoor workers and the elderly.';
    }
    // Humanitarian response
    if (/relief|ReliefWeb|cluster|displaced|shelter|response|evacuat/i.test(haystack)) {
      return '🚨 Confirm the operation status on ReliefWeb and route requests through the standing cluster coordination.';
    }
    return `Verify against a primary Nepali source before acting on ${ticker || 'this read'}.`;
  }
  ...
  _ruleBasedEvaluation(signals, delta) {
    ...
    if (seismicSignal) {
      return { shouldAlert: true, tier: 'FLASH', confidence: 'HIGH', headline: 'Significant Earthquake Detected',
        reason: 'USGS has recorded an M5.5+ event in the Nepal region.', actionable: 'Check epicentre and depth. Verify highway status.',
        signals: ['seismic_event'], crossCorrelation: 'USGS seismic' };
    }
    ...
    if (criticalQuake && criticalWater) {
      return { shouldAlert: true, tier: 'FLASH', confidence: 'HIGH',
        ...
        reason: 'Seismic activity and extreme rainfall escalating together. Slopes loosened by shaking fail under saturation.',
    ...
    if (extremeNew && (extremeNew.severity === 'critical' || extremeNew.severity === 'high')) {
      return { shouldAlert: true, tier: 'FLASH', confidence: 'HIGH',
        ...
        reason: `Extreme-severity weather alerts rose ${extremeNew.from} → ${extremeNew.to}.`,
        actionable: 'Cross-check DHM bulletins and confirm district warnings have gone out.',
        signals: [extremeNew.label || extremeNew.key], crossCorrelation: 'Open-Meteo severe weather' };
    }
    ...
    if (escalatedHighs.length >= 2) {
      return { shouldAlert: true, tier: 'PRIORITY', confidence: 'MEDIUM',
        ...
        reason: `Multiple hazard indicators escalating: ${escalatedHighs.map(s => s.label || s.key).slice(0, 3).join(', ')}.`,
        actionable: 'Monitor for continuation.',
        signals: escalatedHighs.map(s => s.label || s.key).slice(0, 5), crossCorrelation: 'multi-hazard' };
    }
    ...
    if (newDisaster) {
      return { shouldAlert: true, tier: 'PRIORITY', confidence: 'MEDIUM',
        ...
        reason: `ReliefWeb active disasters moved ${newDisaster.from} → ${newDisaster.to}.`,
        actionable: 'Route district requests through the standing cluster coordination.',
        signals: [newDisaster.label || newDisaster.key], crossCorrelation: 'UN OCHA ReliefWeb' };
    }
    ...
    if (escalatingFire.length >= 2) {
      return { shouldAlert: true, tier: 'PRIORITY', confidence: 'MEDIUM',
        ...
        reason: `${escalatingFire.map(s => s.label || s.key).join(', ')} all escalating.`,
        actionable: 'Expect valley AQI to worsen and hill-airstrip visibility to drop.',
        signals: escalatingFire.map(s => s.label || s.key).slice(0, 4), crossCorrelation: 'FIRMS + air quality' };
    }

    if (criticals.length >= 1 || highs.length >= 3) {
      const top = criticals[0] || highs[0];
      return { shouldAlert: true, tier: 'ROUTINE', confidence: 'LOW',
        headline: top.label || top.reason || 'Hazard Change Detected',
